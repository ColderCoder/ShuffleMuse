...
按 `cover.jpg` → `cover.png` → `folder.jpg` → `folder.png` 返回指定相对目录同级、大小写不敏感的封面。`dir` 必须恰好出现一次并使用干净的相对路径；路径穿越、绝对路径、反斜杠和任一目录 symlink 返回 `400 INVALID_DIR`。目录不存在或没有可用封面返回 `404 COVER_NOT_FOUND`。该端点不会递归查找子目录、扩展到其他文件格式或探测音频内嵌封面。
